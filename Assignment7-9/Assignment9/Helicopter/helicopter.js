...
(function () {
    ...
    Copter = function Copter(name) {
        ...
        this.position = [ 3,1.8,3];    // will be set in init
        //this.color = [.9,.3,.4];
        this.color = [1, 1, 1];
    ...
    Copter.prototype.init = function(drawingState) {
        ...
        if (!copterBodyBuffers) {
            var arrays = {
                //vpos : { numComponents: 3, data: helicopterVertex },
                vpos : {numComponents: 3, data: fuselageVertex.concat(door1Vertex.concat(door2Vertex.concat(door3Vertex.concat(door4Vertex.concat(door5Vertex)))))},
                vnormal : {numComponents:3, data: fuselageNormal.concat(door1Normal.concat(door2Normal.concat(door3Normal.concat(door4Normal.concat(door5Normal)))))},
                //indices : helicopterIndex
            };
            copterBodyBuffers = twgl.createBufferInfoFromArrays(drawingState.gl,arrays);

            var rarrays = {
                vpos : {numcomponents:3, data: main_rotorVertex},
    ...
    Copter.prototype.draw = function(drawingState) {
        ...
        modelM = twgl.m4.multiply(twgl.m4.scaling([.15, .15, .15]), modelM);
        modelM = twgl.m4.multiply(twgl.m4.rotationX(-Math.PI/2), modelM);
        ...
        modelM = twgl.m4.multiply(twgl.m4.rotationZ(drawingState.realtime/50), modelM);
    ...
    Helipad = function Helipad(position, size) {
    ...
    function advance(heli, drawingState) {
        ...
        switch (heli.state) {
            ...
            case 3: // fly towards goal
                ...
                break;
            case 4: // land at goal
                var destAlt = heli.lastPad.position[1] + 1 + heli.lastPad.helipadAltitude;
...
grobjects.push(new Helipad([ 3,1.8,4]));
grobjects.push(new Helipad([ 3,1.8,-3]));
grobjects.push(new Helipad([-3,1.8,-3]));
grobjects.push(new Helipad([-3,1.8,2]));
